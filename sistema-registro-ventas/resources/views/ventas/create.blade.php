...
@section('content')
                ...
                <table class="w-full text-sm text-left text-gray-500" id="productos-table">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="px-6 py-2">
                                <button type="button"
                                    class="inline-flex items-center px-3 py-1.5 text-sm font-medium text-red-800 bg-red-100 rounded hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-red-400 remove-row">Eliminar</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
    ...
    <script>
        ...
        document.getElementById('add-producto').addEventListener('click', () => {
            ...
            newRow.innerHTML = `
        <td class="px-6 py-2">
            <select name="productos[${rowCount}][producto_id]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                @foreach ($productos as $producto)
                    <option value="{{ $producto->id }}">{{ $producto->nombre }} - ${{ $producto->precio }} - {{$producto->stock}} en stock</option>
                @endforeach
            </select>
            @error('productos.[${rowCount}].producto_id')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </td>
        <td class="px-6 py-2">
            <input type="number" name="productos[${rowCount}][cantidad]" min="1"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
            @error('productos.[${rowCount}].cantidad')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </td>
        <td class="px-6 py-2">
